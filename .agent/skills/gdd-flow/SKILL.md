...
Two habits keep `next:` renderable, both learned from arcs that drifted: it is one line of **10–20 words**, and it is a *next step*, not a status report. The moment it starts accumulating "DONE X, then Y, OLD NOTE: …" it has become a status dump — move that history into the body and leave the pointer. Quote the value and avoid embedding a `"` inside it; an unescaped quote closes the YAML scalar early and takes the **whole file's** frontmatter down with it, silently removing every arc on that host from the dashboard. `ws hoard lint` catches the broken quote (as unparseable frontmatter) and an overlong or multi-line `next:` — but it measures characters against `WS_ARC_NEXT_MAX`, not the 10–20 word budget, and it cannot tell a next step from a status report. Those two stay your judgment.
