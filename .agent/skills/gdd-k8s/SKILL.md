...
| `ws k8s <built-in write verb>` with `--dry-run=client` or `--dry-run=server` targeting in-scope namespace | The wrapper validates it as a distinct scoped dry-run and lets it proceed without write friction; namespace, context, manifest, and cluster-scope checks still apply. |
