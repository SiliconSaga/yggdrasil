...
Beyond triage, the arcs walk also surfaces prioritization moves
when the SP-A optional fields are populated:

- **Act-order walk** (when `impact:` / `urgency:` are present).
  Read `impact:` (high/medium/low) and `urgency:` (asap/next/soon/later)
  across the current host's arcs and propose an act-order — high impact
  × `asap` first, then high × `next`, then medium × `asap`, and so on.
  Surface as a brief ranked list ("if you have an hour, look at X").
  Propose-then-confirm; no automatic re-sort, no auto-write to `arcs:`.
  Arcs without either field set are skipped — the walk is opt-in per
  arc. Arc-level mirror of the scribe micro-ceremony.
- **Priority collapse** (arcs with a `project:` link). When an arc
  carries `impact:`, `urgency:`, and a `project:` field pointing at a
  vault project note, propose updating that project's lightweight
  `priority:` field to reflect the arc's richer two-dimensional read.
  Illustrative mapping: any × `asap` or high × `next` → `high`; high ×
  soon/later or medium × asap/next → `medium`; otherwise → `low`. The
  collapse is **drift-tolerant** — no invariant, no mechanical sync.
  Propose only when the project note's current `priority:` looks out
  of step with the arc, only on the human's go-ahead. Cross-repo move
  (thalami hoard → vault hoard); the scribe skill picks up the same
  proposal from the project-note side (see @scribe). See the [SP-A
  design](https://github.com/SiliconSaga/yggdrasil/blob/main/docs/plans/2026-05-19-arc-dashboard-qol-design.md)
  for the rationale.
