...
- **`ws clone-fork <component>/<repo>`** — fork-and-PR for a nested repo *in place*. A module only compiles inside the engine tree it was cloned into, so cloning one out to `components/` yields a checkout that cannot build; nested mode therefore adopts the existing checkout and wires the fork remote rather than cloning. The upstream comes from the repo's own `origin`, since a nested repo is never declared in ecosystem config (#155).
- **`commands.testFilter` — `ws test <comp> <name>` for runners it cannot infer.** Gradle, Go, pytest and unittest have guessable filter syntax; a make target or npm script does not, so the adapter declares its own with `{}` marking where the selector goes. Opt-in on purpose: without it a filter is still refused rather than dropped, so a green full suite is never mistaken for the one test asked for (#165).
- **`ws checkout <target> <branch> [-b]`** — switch or create a branch through the wrapper, nested targets included. Branches only, deliberately: `git checkout -- <path>` restores files and discards working-tree changes, so the verb is built on `git switch`, which has no path mode at all, and refuses `--` before git sees it (#155).
...
- **`ws hoard lint`** — validates thalamus frontmatter across every host in a thalami hoard: that the block parses at all, that each arc carries the required keys, that `status` is one of the values the dashboard can render, and that `next` is a single line short enough for its table cell (`WS_ARC_NEXT_MAX`, default 200). Reports in the `key: value` shape of `ws hoard cadence`; exits 0 clean, 1 on findings, 2 on tooling failure — a missing `yq` included. An explicit hoard argument must be a plain directory name, since the verb is pre-allowed. `gdd-orientation` runs it as a session-start warning and `gdd-housekeeping` at the head of its arc walk. See [hoards.md](docs/gdd/hoards.md#frontmatter-lint--ws-hoard-lint) (#162).
- **`[headless-allow]` ships its first two entries** — `identify` and `file`, so a sandbox handed a multi-megabyte phone photo can measure it instead of committing it blind. `file` is allowed only as `file -- <path>`, because `file -C` compiles a magic file and writes one; `convert` stays out because it writes wherever it is pointed, which belongs behind a verb that can validate a destination. Branch work needs no entry now that `ws checkout` reaches this tier already allowed (#164).
...
- **`ws clone` and `ws clone-fork` make blobless partial clones** (`git clone --filter=blob:none`). Trees and commits still arrive up front, so `log`, `blame` and `diff` work at once; file contents fetch on checkout and on the first read of an older revision. A large history (DestinationSol) went from minutes to seconds. The cost is a network round-trip the first time old content is read, so history is not fully available offline; `WS_CLONE_FILTER=` (empty) restores a full clone, and any other value is passed to git as the filter spec (#159).
...
- **`ws review --since last-push` no longer fails when GitHub's events feed has not caught up, nor answers with a stale push when the feed missed a force-push.** That feed is lossy and routinely minutes late, which is exactly when the flag gets used, and it can keep an older push to the branch while never receiving the one that replaced it — measured on #155 as a timestamp three weeks old that made a whole fixed round read as new. A feed push now counts only when the head it names is still the branch head; otherwise, and when the feed is empty, the lookup falls back to the head commit's earliest check-suite creation time, which GitHub stamps at push and a PR author cannot forge, and only then to all history with a note. `prev-push` keeps its all-history fallback rather than borrowing the head's stamp. GitLab, which exposes no push timestamps, now takes the same all-history path with a note instead of erroring.
- **`ws review` can no longer hang on a credential prompt.** Its base-branch drift check runs a real `git fetch`, and a fetch that gets a 401 falls into whatever prompt the shell inherited — a terminal opened from an IDE exports a GUI `GIT_ASKPASS` that waits forever, and the whole `ws review` (or a test suite driving it) stalls with it. The fetch now injects the provider token when one applies and closes every prompt path (terminal, askpass, Git Credential Manager), so it fails fast and the check degrades to silence as it was always meant to.
- **A NUL byte can no longer hide a `kubectl` call from the Kubernetes guard.** Binaries are identified by executable magic (ELF, PE, Mach-O) rather than by `grep`'s NUL-based binary heuristic, which an ordinary shell script could trip with one NUL in a comment — becoming invisible to the content scan while still running normally under bash. Anything not a recognised executable format is scanned with NULs stripped, so an unknown format is inspected rather than skipped, and a file that cannot be read now fails closed instead of reading as "no kubectl" (#168).
- **A compiled binary is no longer read as a script that calls `kubectl`.** The Kubernetes guard's content scan matched the token anywhere in an executed file, and a Go binary linking client-go carries it in its rodata — so with a scope armed, `kustomize build` was denied with advice to "run each step via `ws k8s`" for a command that never invoked kubectl. `helm`, `k9s` and `flux` were blocked the same way. Binaries are now skipped; text scripts are inspected exactly as before, and the three copied call sites share one helper so the Claude and Codex hooks cannot drift apart (#168).
- **`ws review`'s Index counts CodeRabbit's "Outside the diff" findings again.** CodeRabbit started heading the block that hides findings inside the review body "Outside the diff (N)" as well as the older "Outside diff range comments (N)" — both are in use on reviews from the same week — so the "Special finds" nudge silently stopped firing on some reviews while the finding itself still rendered further down. Both headings are matched now (#169).
...
- **An editor's `GIT_ASKPASS` no longer hangs `ws` indefinitely.** VS Code and Cursor export `GIT_ASKPASS` into every integrated-terminal shell (`git.terminalAuthentication`, on by default), pointing at a helper that blocks on a GUI dialog nobody answers in an agent session. `GIT_TERMINAL_PROMPT=0` does not govern askpass — git consults it *before* the terminal — so any git call needing a credential hung forever instead of failing: `ws clone` / `ws pull` / `ws push` on a missing or expired token, and `ws review`'s branch-drift fetch. Observed twice on one machine, once for 14h59m. Every git call `ws` makes now runs with `GIT_ASKPASS` and `SSH_ASKPASS` cleared, tokenless paths included. The terminal prompt is deliberately left intact on the tokenless path, so a human still authenticates interactively while an agent fails fast (#163).
- **`ws preflight` reports an inherited GUI askpass** under a new **Environment:** section, naming the editor setting to turn it off — raw `git` in the same shell stays exposed even though `ws` protects its own calls. Advisory: it never changes the exit code. [Troubleshooting](docs/git-provider-setup.md#a-git-command-hangs-forever-with-no-output-vs-code--cursor-terminals) covers diagnosis and the permanent fix (#163).
- **The plain `ws cr <comp> edit <n> <bodyfile>` form no longer aborts on macOS.** Expanding an empty array under `set -u` is an "unbound variable" error on bash 3.2.57, so the edit ran only when at least one of `--remote` / `--title` / `--upstream` was also passed; `ws cr <comp> --upstream` with no other arguments likewise died before printing its usage message. Both expansions are now guarded (#163).
- **`ws issue` and `ws issue <comp> edit` match a named remote literally.** The name was interpolated into a basic regular expression, so a value carrying regex metacharacters could select a remote other than the one named — deciding which repository an issue is filed against or edited in. Matching is now fixed-string, whole-line and locale-stable (#163).
- **A thalamus with unparseable frontmatter no longer fails silently.** The Arc Dashboard selects files with `WHERE arcs`, so a file whose frontmatter does not parse simply stops matching — every arc on that host vanishes from the cross-host view while the file still reads correctly to a human or an agent. The arc schema had three enforcement layers and all three were prose; nothing had ever parsed the YAML. `ws hoard lint` reports it, and treats it as the severe case rather than as a missing field (#162).
- **`last_touched` is now written, not just read.** It feeds the dashboard's decay icons and housekeeping's stale-arc check, and no skill instructed anyone to maintain it. `gdd-flow` and `gdd-orientation` now say to stamp it in the same edit that touches an arc — a wrong freshness signal is worse than an absent one (#162).
