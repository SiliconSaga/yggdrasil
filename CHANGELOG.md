...
- **An editor's `GIT_ASKPASS` no longer hangs `ws` indefinitely.** VS Code and Cursor export `GIT_ASKPASS` into every integrated-terminal shell, pointing at a helper that waits on a GUI dialog. `GIT_TERMINAL_PROMPT=0` does not govern askpass — git consults it *before* the terminal — so any git call that needed a credential blocked forever rather than failing fast: an expired token on `ws push` / `ws pull` / `ws clone`, and `ws review`'s branch-drift fetch on any repo whose credentials were not cached. The token-auth environment and that fetch now clear `GIT_ASKPASS` and `SSH_ASKPASS`, so the drift check degrades to silence as documented and auth failures surface as errors (#163).
