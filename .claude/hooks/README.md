...
Two constraints, both deliberate. `__SANDBOX_TARGET__` in a pattern expands to the `GDD_SANDBOX` value, so the allowance covers only that component; a wildcard there would also match `ws exec yggdrasil …`, the workspace repo itself. And commands are named per verb — `ws exec *` or `bundle exec *` would amount to arbitrary execution. Patterns keeping the placeholder are skipped when the value is missing or is not a component name, so a malformed flag grants nothing. Tier 2 redirects run first and still apply, so wrapped verbs (`git commit`, `git push`, `gh pr create`) cannot be reached this way.

The flag is read from the environment rather than a file in the workspace: an agent cannot alter the environment of the hook its own tool call spawns, but it can write files. Neither constraint is a security boundary — an argument can still name a path outside the component — so the container remains the boundary; this keeps the ordinary working set prompt-free without widening the blast radius by default.
