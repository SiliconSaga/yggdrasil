...
`git checkout` and `git switch` are deliberately not allowed. Both have forms that discard uncommitted work, and a glob cannot separate those from switching branch, so a sandbox cannot change branch without a human until `ws checkout` (yggdrasil #155) exists as a whole verb to allow. Tier 2 redirects run first and still apply, so wrapped verbs (`git commit`, `git push`, `gh pr create`) cannot be reached this way either.
