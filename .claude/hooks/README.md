...
**`[headless-allow]`** — read only when `GDD_SANDBOX` is set in the hook's environment, which a sandboxed session does with the name of the single component it is scoped to. In that mode an ask has no answerer: a permission prompt reaches either nobody or a chat user who cannot evaluate a tool call, so every `[ask-commands]` match becomes a **deny**, and these patterns allow instead. Denying rather than skipping the tier matters — skipping would let destructive commands through unprompted.

Three constraints, all deliberate. `__SANDBOX_TARGET__` in a pattern expands to the `GDD_SANDBOX` value, so the allowance covers only that component; a wildcard there would also match `ws exec yggdrasil …`, the workspace repo itself. The value must be a valid component name and must not be `yggdrasil` — patterns keeping the placeholder are skipped otherwise, so a malformed or over-reaching flag grants nothing. Commands are named per verb and down to the subcommand — `ws exec *`, `bundle exec *`, even `bundle exec jekyll *` would amount to arbitrary or unintended execution. And naming a read verb is not on its own proof that nothing is written: `--output=<path>` is a diff option accepted by `git diff`, `log` and `show` alike, so the hook refuses that argument for this section rather than trusting the verb.

`git checkout` and `git switch` are deliberately not allowed as patterns. Both have forms that discard uncommitted work, and a glob cannot separate those from switching branch. Creating a branch is the exception, because a sandbox cannot open a pull request without one: the hook parses `ws exec <target> git checkout -b <name>` directly, requiring a single ordinary branch name so that a start-point, a `-f` or a `--` pathspec cannot ride along. Switching to an existing branch still needs a human until `ws checkout` (yggdrasil #155) exists as a whole verb to allow. Tier 2 redirects run first and still apply, so wrapped verbs (`git commit`, `git push`, `gh pr create`) cannot be reached this way either.
