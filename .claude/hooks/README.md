...
`git checkout` and `git switch` are absent for a different reason. Both have forms that discard uncommitted work, and a glob cannot separate those from switching branch. Branch work goes through `ws checkout <target> <branch>` instead, with `-b` to create: it is built on `git switch`, which has no path mode, refuses `--` before git sees it, and rejects path-shaped branch names, so it can be granted whole. The hook pins it to the sandbox component, since it is allowed wholesale in `settings.json` and would otherwise reach every component including yggdrasil. Raw `git switch` and the `ws exec <target> git checkout` forms meet a Tier 2 redirect pointing at it; bare `git checkout` is left unredirected because it also restores paths, which `ws checkout` cannot do. Tier 2 redirects run first and still apply, so wrapped verbs (`git commit`, `git push`, `gh pr create`) cannot be reached this way either.
