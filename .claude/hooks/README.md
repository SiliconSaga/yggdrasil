...
**`[headless-allow]`** — read only when `GDD_SANDBOX` is set in the hook's environment, which a sandboxed session does with the name of the single component it is scoped to. In that mode an ask has no answerer: a permission prompt reaches either nobody or a chat user who cannot evaluate a tool call, so **every** path that would prompt becomes a **deny**, and these patterns allow instead. Denying rather than skipping the prompt matters — skipping would let destructive commands through unprompted.

The conversion lives in the hook's `ask()` helper rather than beside the ask-list, so it covers the dozen earlier paths that prompt without ever reaching Tier 4: ambiguous expansion and backslashes, opaque interpreter passthrough, the Kubernetes write floor and its guard failures, sensitive Edit/Write targets, dispatcher containment. Those are all fail-closed, so leaving them out was never an escalation — it was the hang this mode exists to end, still reachable by every route but one. Note the asymmetry: a *malformed* `GDD_SANDBOX` grants no allowances but still converts prompts, because a value the hook cannot resolve still means nobody is there to answer.
...
**The section ships three entries, each exact.** `git checkout main` returns a sandbox to its default branch — spelled literally, with no options and no wildcard, because `git checkout <token>` restores a *path* when the token names one, so a wildcard here would be the discard form wearing a different hat. Switching to any other branch still needs a human. `identify` and `file` read an image and report what it is, so an agent handed a multi-megabyte phone photo can measure it rather than committing it blind. Branch *creation* is parsed in the hook rather than listed as a pattern.
...
**The site build is not granted, and the obvious narrowing does not rescue it.** `jekyll build --destination <path>` cleans that path of files it did not generate, so the flag was pinned out and only the bare command allowed — which is still unsafe, because Jekyll also reads `destination` from `_config.yml`, an ordinary component file the agent edits. The bare command can therefore clean the component root or the workspace with no flag involved. A glob cannot see a value that lives in a file, so no pattern here can make this safe; `ws build`, resolving the effective destination and refusing one outside the component, is the fix. Until then the sandbox builds through CI on the pull request, which is where the preview and visual diff come from anyway.

**Git reads are not listed here, and adding them back would be a bug.** The validated-read fast path already allows them in every session, headless or not, after checking the target is a declared component, the tokens carry no globs, the verb is on its allowlist and the shape is safe. A Git command therefore only *reaches* this section when that validator refused it — so a `git diff*` pattern would not make ordinary reads frictionless (they never arrive), it would re-grant precisely the shapes the validator turned down: `git diff --no-index <a> <b>` reads outside the component, `git difftool -x <prog>` runs an arbitrary program, and `git diff*` matches `git difftool` besides.

`git checkout` and `git switch` are absent for a different reason. Both have forms that discard uncommitted work, and a glob cannot separate those from switching branch. Creating a branch is the exception, because a sandbox cannot open a pull request without one: the hook parses `ws exec <target> git checkout -b <name>` directly, requiring a single ordinary branch name so that a start-point, a `-f` or a `--` pathspec cannot ride along. Switching to an existing branch still needs a human until `ws checkout` (yggdrasil #155) exists as a whole verb to allow. Tier 2 redirects run first and still apply, so wrapped verbs (`git commit`, `git push`, `gh pr create`) cannot be reached this way either.

The flag is read from the environment rather than a file in the workspace: an agent cannot alter the environment of the hook its own tool call spawns, but it can write files. None of this is a security boundary — an argument can still name a path outside the component — so the container remains the boundary; this keeps the ordinary working set prompt-free without widening the blast radius by default.
