...
Subcommands that take a target (commit, push, cr, issue, review, log, diagnose, test, lint, fmt, build, run, clean) also accept realm and hoard names, not just components.

**Adapter-routed verbs — consult `ws orient` first:** `ws test` / `ws lint` / `ws fmt` / `ws build` / `ws run <comp>` / `ws clean <comp>` (bare `ws clean` sweeps workspace drafts instead).
