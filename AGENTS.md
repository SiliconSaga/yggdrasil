...
8. **Write to a length someone will actually read.** `style.changeNotes` budgets commit, CR and issue bodies, and `ws commit` reminds you when one runs over. Nothing budgets the rest — code comments, doc prose, dashboard and hover text, PR review comments, chat replies — and that is where length quietly accumulates. Default to the shortest version that still carries the evidence, the trap, and why it matters; drop anything that restates what the reader is already looking at. Three specific habits to avoid, because they are the ones that actually happen: narrating a diff the reader can see, restating the request before answering it, and closing with a summary of what was just said. Hover text and UI strings get one sentence. If something genuinely needs length — an intricate mechanism, a decision with real alternatives — give it the length and say why, rather than padding everything to be safe.
