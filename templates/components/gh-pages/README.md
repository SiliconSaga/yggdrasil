...
Local preview needs **Ruby + Jekyll**, which `ws preflight` deliberately does *not* check (they're a this-template need, not a workspace prerequisite). Install Ruby (Windows: `winget install RubyInstallerTeam.RubyWithDevKit.3.3` — the DevKit-bundled build avoids an interactive post-install step; macOS/Linux: your package manager or a Ruby version manager), open a fresh shell so PATH picks it up (restart your whole IDE if you work inside one — it hands terminals its launch-time PATH), then `gem install jekyll` and run `jekyll serve` from the component directory.

The scaffold ships no `Gemfile`, so plain `jekyll serve` is the right form. If you later add one (to pin versions or match GitHub's build with the `github-pages` gem), switch to `bundle install` + `bundle exec jekyll serve` — and avoid the `wdm` gem (or pin it above 0.1.1): `wdm 0.1.1` fails to compile on Ruby 3.3+, and it's only a file-watching optimization the preview works without.
