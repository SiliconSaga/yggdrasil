...
- **Adding `scripts/` persistently.** Inside Git Bash, the `~/.bashrc` export above is enough. The `ws` entry point is a Bash script, so adding `scripts/` to the Windows PATH alone does not create a native cmd/PowerShell command. From cmd or PowerShell, enter Git Bash or invoke the CLI through Bash from the workspace root, for example `bash scripts/ws help`.
- **Git Bash mangles Unix-style paths passed to native `.exe`s.** When you hand a container-absolute path (e.g. `/data`, `/opt/render`) to a Windows-native executable like `docker`, `kubectl`, or `nerdctl`, MSYS auto-rewrites it into a Windows path (`C:\Program Files\Git\data`), breaking the argument. The variable that disables this is `MSYS_NO_PATHCONV=1`. For docker specifically, the clean fix is **`ws docker <args…>`** — it scopes `MSYS_NO_PATHCONV=1` to that one call on Windows and is a transparent passthrough everywhere else, so nothing global changes and no other tool is affected. If you do heavy container work and want the override always-on, you *can* set it globally instead — `export MSYS_NO_PATHCONV=1` in `~/.bashrc`, or add `{ "env": { "MSYS_NO_PATHCONV": "1" } }` to the `env` block of `.claude/settings.local.json` so every Bash call (including subagents) inherits it. The `ws` CLI is hardened to work either way (it routes its `yq`/`gh` file paths through `cygpath`), so a global setting no longer breaks `ws commit`/`cr`/etc. — but it is a broad change that disables path conversion for *all* native-exe calls, so prefer `ws docker` unless you specifically want the global behavior. Off Windows there is no MSYS conversion, so all of this is inert.
- **Paths meant for *inside* a container or pod need a different fix — not the variable.** The wrappers above handle paths that name something on your machine. They do not help when the path names a location inside a container, because the tool cannot tell the two apart. `ws k8s exec -n ns deploy/x -- cat /etc/app/config.yml` arrives as `cat 'C:/Program Files/Git/etc/app/config.yml'`, and `ws k8s` does not scope `MSYS_NO_PATHCONV` the way `ws docker` does. Reach for the portable idiom instead of an env var: **wrap the remote command so no argument begins with `/`.**

  ```bash
  # mangled — the argument starts with / and MSYS rewrites it
  ws k8s exec -n ntfy deploy/ntfy -- cat /etc/ntfy/templates/heimdall.yml
  # correct — the path is inside a quoted string, so MSYS never sees it
