...
3. File GitHub issues for anything delegatable. Every issue body must start with the AI attribution line (see `gdd-github-issues` skill). Include what the problem is, the expected outcome, and any relevant file paths or context a fresh agent would need.
