...
The yggdrasil workspace runs two distinct permission systems that answer different questions:
...
Both layers must approve an action that crosses both surfaces: a `git push` allowed locally (in the matcher's auto-allow set or an explicit pattern) is still rejected remotely if the PAT lacks push scope or the agent identity isn't a collaborator on the repo. Layered defense.

This doc covers the remote layer: who the agent is, what tokens it holds, and how the workspace's fork-and-PR pattern keeps human and agent contributions attributed.
...
1. **Reviewable attribution.** Every commit, PR, and review thread is authored by a human or an agent. Co-Authored-By trailers in commit messages preserve the pairing; the actual `Author:` field reflects whoever ran the operation.
2. **Scope minimization.** The agent PAT holds *just enough* scope for routine work (`repo`, plus a couple of read scopes). The human PAT might hold more (workflow editing, package publishing, etc.). Compromise of the agent token is bounded.
3. **Accountability and revocation.** If something goes wrong, revoke the agent token without affecting your own Git access.

Setup mechanics — installing the CLI tools, generating tokens, loading them via `.env` — live in [`docs/git-provider-setup.md`](../git-provider-setup.md). This doc focuses on the *patterns* that compose those tokens with the fork-and-PR model.

---

## 2. The fork-home pattern

For repos owned by an org you're not a maintainer of (external open-source contribution), the workflow is:
...
A complex local checkout may have several remotes: an external source project, an internal mirror or company home, a fork-group home, and an arbitrary extra remote. The workspace treats the fork remote as first-class, one non-fork remote as the source-project CR target, and extra remotes as explicit operator choices.
...
The homes model makes the fork home first-class through `identity.homes.fork.namespace`. The `homes.internal`/`homes.external` routing distinction remains future design work; until then, use `defaults.upstreamRemote` for the one non-fork source-project remote `ws cr --upstream` should target, `ws cr --remote` for rare alternate fork/head remotes, and treat any fourth remote as an explicit escape hatch rather than workspace policy.
...
The agent PAT's scope set determines what it can do remotely. The recommended baseline is one write scope plus three read scopes — low-risk additions that remove recurring papercuts in routine PR/review operations:
...
`workflow` scope is needed only if the agent edits files under `.github/workflows/`.

The three `read:*` scopes grant no mutation power and unlock no code the agent can't already reach via `repo` — they're metadata reads that gate routine `gh pr edit`-shaped operations. Declining them just means hitting the `gh api PATCH` fallback below for every PR-edit-shaped call.
...
`ws push`, `ws cr`, `ws review`, etc. auto-detect the provider from the component's remote URL and pick the right CLI (`gh` vs `glab`) and token. Mixed-provider workspaces (some components on GitHub, some on GitLab) work without per-command configuration.
...
**Default behavior with no `gitTokens` entry.** For GitHub remotes, `gh` reads `$GH_TOKEN` automatically — no `gitTokens` entry needed in the common case. Same for GitLab: `glab` reads `$GITLAB_TOKEN`. `ws diagnose` reports this as `✓ <VAR> is set (default for <provider>; no gitTokens entry needed)`. An explicit `gitTokens` entry is only needed for **fine-grained per-namespace control** — e.g. org repos use the team's `GH_TOKEN` but personal forks under `<your-user>` use a different `GH_TOKEN_PERSONAL`. Add a longer-prefix `gitTokens` entry to win the longest-prefix match for the personal namespace:
...
**GitLab PATs cannot be downscoped.** A Personal Access Token runs *as you* and inherits your full account access — no way to issue a "read-only on group X" PAT when you're an Owner there. Prefer a narrower GitLab-native actor when your instance supports one.

**GitLab actor ladder.** Use the narrowest actor that can perform the operation:

| Actor | Best fit | Notes |
|-------|----------|-------|
| **Group / Project Access Token** | Self-managed/Dedicated GitLab, or GitLab.com Premium/Ultimate | Explicit role independent of your human account. Good for fork-group write and source-project read tokens. Access-token bot users are scoped to the project/group that created them and can't be invited directly to unrelated groups. If the source project/group invites the fork-home group, a fork-group access-token bot can gain source-project read through that membership and satisfy GitLab's fork API. |
| **Group Service Account** | GitLab.com Free/Premium/Ultimate, or self-managed instances exposing group service accounts to top-level group Owners | Good fork-home actor when it can read the source project and create the fork in a non-personal namespace. Can only be added to its creation group or descendants, so sibling/external source projects must be public, same-hierarchy readable, or shared to the fork-home group. Service-account forks must target a group/project namespace, not personal. |
| **Instance Service Account** | Admin-managed self-hosted/Dedicated instances | Closest to a general robot user — can be invited across groups. Too heavyweight for normal per-user GDD setup. |
