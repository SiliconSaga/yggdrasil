...
- **Chat-channel agents** — GDD driven from a phone over chat. Claude Code's official channel plugins (Discord, Telegram, iMessage) turn a direct message into an agent session: the user describes a change, the agent opens a PR, CI builds the preview and diff, the bot replies with a link and the diff image, and a reply of "ship it" merges. The user never touches git directly; the PR remains the audit trail underneath. Voice comes free from the phone's own dictation — no bespoke voice feature needed.
- **Scoped agent sandboxes, trust-scaled** — running an always-on agent *for someone else* means containing it, and the rigor should scale with trust. For a trusted user: a plain container with credentials mounted read-only and a tight, deny-biased policy — the mount boundary alone means the agent cannot see what is not mounted. For the untrusted case: **NVIDIA OpenShell** (Apache-2.0), whose policy engine and *egress token injection* keep real credentials outside the container entirely, so a prompt-injected agent has nothing to leak. The same purpose-built image drops into either. Multi-tenant follows a *bring-your-own-entitlement* model — each user runs on their own subscription — so nothing is shared and nothing is resold.
