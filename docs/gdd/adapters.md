...
`ws test <comp> <name>` narrows to one test where the runner's filter syntax is known (Gradle, Go, pytest, unittest). For any other runner the adapter declares its own as `commands.testFilter`, with `{}` marking where the selector goes, e.g. `testFilter: "make test-one NAME={}"`. Without it a filter is refused rather than silently dropped.
