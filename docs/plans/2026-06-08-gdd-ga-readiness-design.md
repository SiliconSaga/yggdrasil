...
- Main agent: the orientation skill's "CLAUDE_MODEL refresh (main agent only)" step rewrites the `.env` default to the running model.
- Sub-agent: `ws commit --help` now explicitly documents the inline override (`scripts/ws-commit.sh:49-50`: *"Inline is correct for sub-agents — a shared .env rewrite from parallel sub-agents would race"*) with the `CLAUDE_MODEL="Sonnet 4.6" ws commit …` example, and the orientation skill tells sub-agents to skip the `.env` rewrite and use that inline form.
...
### P8 — Fresh-machine first-run papercuts (Nano76Win11 dogfood) — 🟡 One code fix left
Residual findings from the `B2` fresh-Win11 dogfood runs (2026-06-21 → 2026-07-03):
1. **`ws status` errors on a components-less workspace** — with no components cloned it prints the yggdrasil + hoard lines but also emits `Error: cannot get keys of !!null, keys only works for maps and arrays`: `scripts/ws-status.sh:70` runs `yq '.components | keys'` unguarded against a null/missing components map. Cosmetic but it's first-run noise on the exact first command a newcomer is told is safe. **The remaining code fix.**
...
**Sequencing note (2026-07-09):** what's actually between here and the tag: (1) **issue #122** — clone-fork GitHub provider path + default-token fallback + `gitlab-auth --help`, re-verified still valid against current code (glab hard-dep, GitLab-only fork API/URL, no token fallback, no `--help` short-circuit all present); (2) the **P8.1** `ws status` null-components guard; (3) the **B2/B5** one-shot newcomer run on the second clean Windows machine, gh-pages tutorial included; then (4) the **B6** release ceremony per `docs/gdd/versioning.md` (curate `[Unreleased]` → `1.0.0`, annotated tag, push). P6/P7 are filed as post-GA issues #124/#125; the R1 allowlist question is #127; session-liveness was resolved by decision (no liveness marker by design — staleness never gates; `ws clean --sessions-all` is the stale-scope remedy, per the rationale comment in `scripts/ws-k8s.sh`).
