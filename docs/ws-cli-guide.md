...
5. **Bash 4+ is the supported floor, but don't rely on it in code** — `ws preflight` requires bash ≥ 4.0 and Git Bash on Windows and Linux distros qualify, while macOS's system bash is 3.2.57 and always will be (Apple froze it in 2007 because bash 4.0 relicensed to GPLv3, then made zsh the default login shell, so nothing in daily use points at it). Preflight is advice, not enforcement: the permission hook and the dispatcher run on every tool call long before anyone reads a preflight hint. So keep new code to 3.2-safe idioms — no `mapfile` / `readarray`, no `${var,,}` / `${var^^}` (fold with `LC_ALL=C tr` instead), no associative arrays, no `local -n` namerefs, and `${arr[@]+"${arr[@]}"}` for possibly-empty arrays. This restores the original `ws` design constraint (`docs/plans/2026-03-11-ws-cli-design.md`), which had drifted. The `portability:` test in `tests/hook/gdd-permission-hook.bats` pins the hook's half; the rest is convention. The cost of getting this wrong is not a clean error — a `local -n` in the hook's token walk spun a loop forever and hung every git/gh/glab call with no diagnostic at all.
