...
There is **no silent no-trailer fallback**: an agent always has a session id (so it lands on rung 3 and can never quietly skip attribution), and a no-session caller must mark itself with `--human` rather than be guessed at. There is no longer an `identity.co_authored_by`, `CLAUDE_MODEL`, `.env`-sourced, or inline-env `GDD_CO_AUTHOR` rung — the environment is never consulted for identity (see [the design doc](plans/2026-06-15-multi-agent-attribution-design.md)). The resolved value must include an email in angle brackets (e.g. `Codex GPT-5 <noreply@openai.com>`), only feeds the trailer string, and is newline-sanitized — never evaluated as a command. Run `ws whoami` to see who the current session commits as.
